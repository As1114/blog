...
// 使用 React.memo 包装，减少不必要的重新渲染
const PinContainer = React.memo(
  ({
    children,
    href,
    className,
    ...
    // 使用 useCallback 缓存事件处理函数，防止每次渲染重新定义
    ...
    return (
      ...
      >
        <div
          ...
          className="h-[250px] w-[300px] absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2"
        >
          <div
            style={{ transform }}
...
// 使用 React.memo 包装 PinPerspective，防止不必要的重渲染
const PinPerspective = React.memo(({ href }: { href?: string }) => {
  const animations = Array.from({ length: 3 }).map((_, index) => (
    <motion.div
      key={index}
